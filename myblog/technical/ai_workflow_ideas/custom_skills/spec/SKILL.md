---
name: spec
description: Spec-driven feature workflow (spec, then plan, then tasks, then TDD implement), resumable from where it dropped off.
...
One task at a time, top to bottom. For any task with a paired failing-test task, follow the `mattpocock-skills:tdd` red-green-refactor loop. For non-test-first tasks, just implement and verify (build/run as appropriate).
