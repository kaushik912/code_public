...
def extract_java_fields_and_methods(code: str) -> dict:
    """Extract field declarations and method declarations from Java code and return as JSON format."""
    ...
    def visit_node(node: Node):
        """Recursively visit nodes to find field and method declarations."""
        ...
        # Look for method declarations
            ...
            for child in node.children:
                if child.type == 'modifiers':
                    # Extract modifiers like private, public, static, final
                    for modifier in child.children:
                        if modifier.type in ['private', 'public', 'protected', 'static', 'final', 'abstract', 'synchronized']:
                            modifiers.append(code[modifier.start_byte:modifier.end_byte])
                elif child.type in ['type_identifier', 'generic_type', 'array_type', 'integral_type', 'floating_point_type', 'boolean_type', 'void_type']:
                    ...
                    for param_child in child.children:
                        if param_child.type == 'formal_parameter':
                            ...
                            for param_grandchild in param_child.children:
                                if param_grandchild.type in ['type_identifier', 'generic_type', 'array_type', 'integral_type', 'floating_point_type', 'boolean_type']:
                                    param_type = code[param_grandchild.start_byte:param_grandchild.end_byte]
                                elif param_grandchild.type == 'identifier':
                                    param_name = code[param_grandchild.start_byte:param_grandchild.end_byte]
...
def main():
    parser = argparse.ArgumentParser(description="Extract field and method declarations from Java code")
    ...
    parser.add_argument("--include-modifiers", action="store_true", 
                       help="Include field and method modifiers in output")
    parser.add_argument("--fields-only", action="store_true",
                       help="Extract only fields (original behavior)")
    parser.add_argument("--methods-only", action="store_true",
                       help="Extract only methods")
    ...
    result = extract_java_fields_and_methods(code)
    
    # Filter output based on flags
    if args.fields_only:
        output_data = result["fields"]
    elif args.methods_only:
        output_data = result["methods"]
    else:
        output_data = result
    ...
    if not args.include_modifiers:
        if args.fields_only:
            ...
            for method in output_data:
                if 'modifiers' in method:
                    del method['modifiers']
        else:
            for field in output_data.get("fields", []):
                if 'modifiers' in field:
                    del field['modifiers']
            for method in output_data.get("methods", []):
                if 'modifiers' in method:
                    del method['modifiers']
    ...
    if args.output:
        try:
            with open(args.output, 'w', encoding='utf-8') as f:
                f.write(output)
            print(f"Java elements extracted and saved to {args.output}")
        except Exception as e:
